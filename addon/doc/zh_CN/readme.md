...
使用 **Ollama Vision** 可以进行图像描述、OCR、图表或数学公式识别。使用可浏览消息显示纯文本结果时，支持的数学公式和表格可供屏幕阅读器导航。
...
为**描述剪贴板内的图像**分配手势后，按一次打开识别结果文档，快速连按两次返回纯文本。
...
对于网页图形，Vis Aware 通常使用对象提供的图像 URL；没有可用 URL 时改用对象截图。启用**优先对网页图像对象截图**后，会先识别屏幕上显示的内容，截图失败时再使用 URL。
