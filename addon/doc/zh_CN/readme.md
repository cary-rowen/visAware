...
自动识别默认关闭。在**自动识别**面板中，可以选择 OCR 或图像描述，再选择当前引擎或一个已启用的特定引擎。提示词和模型仅覆盖该引擎用于自动识别时的设置。提示词留空会使用 Vis Aware 的简洁默认提示词（控制在 20 到 30 字内且不使用 Markdown）；选择**使用引擎设置中选择的模型**会沿用其常规模型。引擎支持时，可以使用**获取模型**加载可用模型名称。

自动识别的引擎列表仅显示支持此功能的引擎。文本验证码识别仅供手动使用。选择“当前引擎”时，如果当前引擎不支持自动识别，自动识别会暂停，切换到支持的引擎后恢复。若原先保存的引擎已无法用于自动识别，设置面板会显示为关闭，您可以重新选择引擎；只有保存设置后才会更新原配置。
...
在支持的系统上，使用 NVDA 2026.3 或更高版本时，即使启用了黑屏或 NVDA 内置放大镜，AI Agent 也可以捕获原始屏幕。启用黑屏后，AI Agent 仍可能将不可见的屏幕内容发送给所选服务，并在用户看不到屏幕时执行操作。Windows Graphics Capture 不可用时，启用黑屏无法启动 AI Agent。
...
使用[白描 OCR](https://baimiaoapp.com) 或白描公式识别时，请在 OCR 引擎设置中选择**登录白描...**，然后输入白描账号的电子邮箱或手机号及密码。两个引擎共用同一登录状态。

Vis Aware 会保存用于识别此设备的固定标识，以及设置按钮显示的昵称或脱敏登录账号和会员等级。这些信息使用 Windows DPAPI 加密保护；密码不会保存。公式识别结果会同时提供文字和公式；在可浏览结果中，公式会以便于屏幕阅读器阅读的方式呈现。

两个引擎都使用[白描 Web 版](https://web.baimiaoapp.com/)。设备数限制、额度和可用功能由白描决定，并可能因账号而异。在 Vis Aware 中退出登录会清除本机保存的登录信息，但不保证同时将设备从白描账号中退出；如不再使用某设备，请在手机版白描中将其退出。

此集成使用白描的非公开 Web 接口。该接口可能在不另行通知的情况下变更或停止服务。使用风险由使用者自行承担；对于账号限制、服务变更、数据丢失或其他因使用该接口产生的后果，Vis Aware 不承担责任。请遵守白描适用的相关条款。
...
在支持的系统上，使用 NVDA 2026.3 或更高版本时，即使启用了黑屏或 NVDA 内置放大镜，手动和自动屏幕识别也可以捕获原始屏幕。Windows Graphics Capture 不可用时，屏幕识别必须关闭黑屏。NVDACN 密码和白描登录会话使用 Windows DPAPI 保护；其他保存的 API 密钥会以明文形式存储在 NVDA 配置中，在创建或分享 NVDA 便携版之前请务必留意数据安全。
